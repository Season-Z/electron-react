...
function renderInfo(list: any, num: number) {
  ...
  for (let index = 0; index < newList.length; index++) {
    ...
    if (isArray) {
      element = [element.label ?? '', element.value ?? '', element.lighthigh ?? '']
    }

    const colItemKey = randomData.toString() + element[0]
    // 一行中的每一项
    const colEle = (
      <div className='colItem' key={colItemKey} style={{ maxWidth: `${100 / num}%` }}>
        ...
        <Tooltip placement='topLeft' title={element[1]}>
          ...
          >
            {/* {element[0] === '促销价' && element[1] === 0
              ? '-'
              : element[0].includes('价')
              ? element[1] / 100
              : element[1]} */}
            {element[1]}
          </span>
...
function DetailInfo(props: DetailInfoProps) {
  ...
  return <div className='detailInfo'>{renderInfo(map, cols)}</div>
}
