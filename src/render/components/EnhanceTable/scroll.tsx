...
import ypRequest, { ResponstResult, Result } from '@utils/ypRequest'
import { useUpDownForInput, useScrollLoad, useCalcScrollY, CalcScrollProps } from './hooks'
...
function EnhanceTable<RecordType = any>(
  ...
  const ref_page = useRef({ current: 1, pageSize: 30 })
  const ref_timer = useRef<any>()
  const ref_result = useRef<Result>({} as any)
  ...
  const [_pagination, set_pagination] = useState<TablePaginationConfig | false | undefined>(
    pagination === false && !scrollable ? false : { ...ref_page.current, ...pagination }
  )
  ...
  const [setScrollEventContainer] = useScrollLoad(bool => {
    if (bool) {
      if (ref_result.current.isEnd) return
      set_pagination(_pagination => ({ ..._pagination, current: (_pagination as any).current + 1 }))
    }
  })
  const { scrollY, setContainer: setCalcScrollYContainer } = useCalcScrollY(scrollOffset)
  ...
  const queryHandle = async (
    ...
    if (url) {
      ...
      if (success) {
        ...
        const list: any = d ?? l ?? []
        ref_result.current = { isEnd: list.length < (_pagination as any).pageSize, ...result }
        if (Array.isArray(list)) {
          setDataSource(
            scrollable && (_pagination as any).current > 1 ? [...(dataSource ?? []), ...list] : list
          )
          setDataTotal(+total)
        }
      }
  ...
  useEffect(() => {
    ...
    if (scrollable) setScrollEventContainer(ref_tableWrap.current)
    if (!scroll?.y) setCalcScrollYContainer(ref_tableWrap.current)
  }, [])
